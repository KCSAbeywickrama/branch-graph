...
This is the Rust implementation: one binary, no runtime to install, and fast enough that
a 284 MB project scans in about a tenth of a second.
...
| --- | --- |
| `src/scan.rs` | Reading one transcript: fork point, active path, and how a branch gets its name |
| `src/tree.rs` | Building the fork forest and the tree art |
| `src/search.rs` | The offline matcher: substring, abbreviation, and the ancestor-keeping filter |
| `src/render.rs` | Color, width and truncation, timestamps, the plain list view |
| `src/tui.rs` | The picker: frame rendering, mouse, and the escape-sequence parser |
| `src/term.rs` | termios, window size, poll/read, signals, local time |
| `src/main.rs` | Arguments, project-dir resolution, the parallel scan, output modes |

Two dependencies, both for the reasons above: `serde_json` to parse transcripts and
`libc` for the terminal syscalls the picker needs.

## Differences from the Node implementation

`--help`, `--json`, `--list`, `<n>` and `..` were diffed byte for byte against the Node
version — output and exit codes — across every project on this machine, the search filter
was compared against the original's own functions over a 118-branch tree, and the picker's
rendered frames were compared frame by frame. What follows is everything that is
deliberately not identical:

- **Faster.** Transcripts are scanned in parallel, and each JSONL line is parsed into a
  narrow struct so the large `message` and tool-result payloads are skipped instead of
  allocated. On a 284 MB project (118 transcripts): full scan ~117 ms versus ~1.4 s, and
  `..` ~9 ms versus ~47 ms.
- **Timestamps are locale-pinned.** The detail panel's full stamp is always
  `M/D/YYYY, h:mm:ss AM/PM`. Node used `toLocaleString()`, which moves with the machine's
  locale.
- **Resize is polled, not signalled.** The picker samples the window size a few times a
  second rather than handling `SIGWINCH`, so a resize repaints within ~200 ms.
- **A damaged transcript degrades instead of failing.** An unreadable or truncated session
  file becomes a branch labelled with its short id instead of taking down the run — which
  matters because every branch forked from it would vanish with it.
- **An out-of-range branch number echoes what you typed.** `branch-graph 99999999999999999999`
  reports that number back; Node printed its float-rounded form.
